...
Edge::Edge() : Edge(nullptr, nullptr, EB, 0, 0) {}
...
const bool Edge::suf(Read* read) const {
  // TODO: provjeri ovu usporedbu
  assert(read != nullptr);
  assert(read == A_ || read == B_);
  if (read == A_) {
    return direction_ == EB;
  }
  return direction_ == BE;
}

const size_t Edge::hang(Read* read) const {
  assert(read != nullptr);
  assert(read == A_ || read == B_);
  if (read == A_) {
    return A_->size() - sizeA_;
  }
  return B_->size() - sizeB_;
}

const size_t Edge::size(Read* read) const {
  assert(read != nullptr);
  assert(read == A_ || read == B_);
  if (read == A_) {
    return sizeA_;
...
const size_t Edge::size() const {
  return (sizeA_ + sizeB_) / 2;
}
